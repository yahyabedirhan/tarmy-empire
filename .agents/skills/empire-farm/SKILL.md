...
3. **Mines, on-role only (decision 013).** Candidates are the planet's role resource only (crystal on :1/:3, metal on :10/:9, deuterium on :14; none on the capital); off-role mines are frozen. A resource whose empire stock exceeds ~24 h of planned spend is not a candidate anywhere (stock rule) — its planets build sinks. Among candidates compute `(cost + share of forced energy) ÷ production_gain_per_hour` from `codex`; the best return in the empire is queued first, wherever it is. The role only breaks ties. Crystal before metal on colonies while empire crystal < ½ metal. A mine whose payback is shorter than the astro ETA (`HANDOFF.md`) is always queued; a crystal-priced non-mine with a longer payback waits.
