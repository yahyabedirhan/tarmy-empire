...
7. **Hand off.** `empire-handover` skill: `HANDOFF.md` reflects this cycle. Commit it. A cycle is complete when every planet file's `wake` names something that has not happened yet, every open mission has a `wake`, and `HANDOFF.md` is committed.
