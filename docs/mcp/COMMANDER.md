...
- `build_ships(key, count, planet_id?)` — ships; defence goes through `build_defense(key, count, planet_id?)` (`build_ships` answers `unknown ship: "rocket_launcher"`, 2026-09-17), into the planet's own *shipyard* line (5 batches, runs beside the construction line — v0.15.3). Solar satellites and crawlers are buildings: `upgrade_building`. Lock: no batch can be ordered while the shipyard or nanite factory is upgrading, and neither can be upgraded while a batch is queued.
