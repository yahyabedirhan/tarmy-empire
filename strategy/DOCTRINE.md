...
| `BOOTSTRAP` | `landing` `powering` `stocking` | planet age < 24 h and the role's exit not met | follow the role's bootstrap order in the `empire-colonize` skill; the empire's surplus feeds it first (decision 013) — a transport every cycle from the nearest stocked planet | role exit met: crystal-world crystal ≥ 5 (metal capped at 6); metal-world metal ≥ 8; deut-world synth ≥ 10 — plus factor 1 and storage/tank ≥ 2 |
...
Roles (long-lived, set in the planet file, decisions 002, 013): `capital` (highest lab, shipyard, fleet home, cargo hulls), `metal-world`, `crystal-world`, `deut-world`, `forward-base`. A planet **produces its role's resource and ships the rest in** (decision 013): off-role mines stay where they are; the role table with each planet's frozen levels is in `strategy/decisions/013-planets-are-organs.md`.
...
**On-role mine first.** A mine level goes only to a planet whose role is that resource, and among those to the one returning the most per 1 000 resources, priced as (mine cost + the share of energy it forces us to buy) ÷ hourly gain — `codex` on every on-role candidate. The capital buys no mines. Source: decision 013 (replacing 008/009's empire-wide ranking, which produced identical colonies and idle metal); BJACK measurements in `ops/diplomacy/2026-09-11_BJACK-chat.md`.

**Stock rule.** A resource whose empire stock exceeds ~24 h of planned spend gets no new mine anywhere; its producers build sinks instead (rocket launchers to the 006 floor, metal-only research, storage if a pool nears cap). Source: decision 013 (1.7M idle metal, 2026-09-17).

**Logistics is a product.** Every planet keeps the cargo hulls its outbound flow needs; the capital builds them. A planet with an empty queue and stock above its next on-role level ships out or builds a sink the same cycle — "waiting for crystal" is not a reason while metal or deuterium sits there. A new colony is the first call on the empire's surplus for its first 24 h. Source: decision 013.
